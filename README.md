...
- Single file uses `statement`; 2–10 images use ordered `statementPages` (the BDO 3-page synthetic fixture uses the latter in ascending page order). After a PNB statement is imported, use **Add continuation page** to append later PNB image pages without re-uploading page 1. Mixing both fields is rejected.
- Files are validated by magic bytes/content, not extension alone. Zero-byte, oversize, path-like names, MIME/signature mismatches, and unsupported types are rejected with stable `code`/`stage`/`requestId` errors — without echoing document contents, stack traces, or filesystem paths. A password-protected statement PDF is opened locally on the first import attempt; the UI then offers a masked, one-request password retry. `pdf_password_required` and `pdf_password_invalid` are returned at `extracting`; `pdf_encryption_unsupported` is returned when the local parser explicitly cannot handle the encryption capability. Passwords are never saved, logged, or included in session data, and `statementPages` never accepts a password field.
